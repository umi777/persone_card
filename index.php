...
$db = new database();
$card_id = (is_numeric($_GET['id'])) ? (int)$_GET['id'] : 1;
...
<?foreach ($person_slider as $slide){ ?>
								<div>
									...
									<div class="slide__status">
										<input type="checkbox" id="like-<?=$slide["ID"]?>" name="like_status" <?=$db->person_slider_liked($slide["ID"]) ? 'checked onchange="dislike('.$slide["ID"].')"':'onchange="like('.$slide["ID"].')"';?>>
